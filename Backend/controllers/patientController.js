...
exports.connectWithDoctor = async (req, res) => {
    try {
        ...
        // Add doctor to patient's connected doctors if not already connected
        if (!patient.connectedDoctors) patient.connectedDoctors = [];
        if (!patient.connectedDoctors.includes(doctorId)) {
            patient.connectedDoctors.push(doctorId);
            await patient.save();
        }

        res.status(200).json({ message: 'Successfully connected with doctor', doctor: { name: doctor.name, specialization: doctor.specialization } });
    } catch (err) {
        console.error(err);
        res.status(500).json({ message: 'Error connecting with doctor' });
    }
};
