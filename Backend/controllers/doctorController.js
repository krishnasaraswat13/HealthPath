...
exports.getDoctorProfileById = async (req, res) => {
    try {
        ...
        // Fetch all reviews for this doctor (increased limit for dashboard display)
...
// PUT /doctor/update-profile — update doctor profile fields
exports.updateProfile = async (req, res) => {
    try {
        const doctorId = req.user.id;
        const allowedFields = ['name', 'specialization', 'fees', 'experience', 'about', 'image', 'availableSlots', 'phone'];
        const updates = {};
...
// GET /doctor/my-patients  — list unique patients from appointments
exports.getMyPatients = async (req, res) => {
    try {
        ...
        for (const apt of appointments) {
            if (apt.patientId && !seen.has(apt.patientId._id.toString())) {
                seen.add(apt.patientId._id.toString());
                patients.push({
