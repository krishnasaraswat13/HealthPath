...
if (!apiKey) {
    console.error("âŒ CRITICAL: GROQ_API_KEY is missing in your .env file!");
} else {
    ...
    try {
        ...
        const completion = await groq.chat.completions.create({
            messages: [
                {
                    role: "system",
                    content: "You are a helpful and empathetic AI health assistant named 'HealthPath AI'. Answer health questions concisely. If the query is serious, advise seeing a doctor."
                },
                {
                    role: "user",
                    content: prompt
        ...
        console.error("âŒ AI Assistant Error:", error.message);
...
exports.summarizeConsultation = async (req, res) => {
    ...
    try {
        ...
        console.error("âŒ Summary Generation Failed:", error.message);
