...
exports.getBookedSlots = async (req, res) => {
    try {
        ...
        // Note: Ensure the date format sent from frontend matches DB exactly (usually YYYY-MM-DD string)
...
exports.rescheduleAppointment = async (req, res) => {
    try {
        ...
        if (!appointmentId || !date || !timeSlot) {
            return res.status(400).json({ message: 'Appointment ID, date, and time slot are required' });
        }
        const appointment = await Appointment.findById(appointmentId);
        if (!appointment) return res.status(404).json({ message: 'Appointment not found' });
